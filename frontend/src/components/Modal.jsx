...
export default function Modal({ isOpen, onClose, title, children, wide, xl, zIndex = 'z-[500]' }) {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
          className={`fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center ${zIndex} p-4`}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          onClick={onClose}
