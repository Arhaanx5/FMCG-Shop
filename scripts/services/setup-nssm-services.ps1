...
if (-not $isAdmin) {
    ...
    $logPath = Join-Path $PSScriptRoot "nssm-setup.log"
    $argList = '-NoProfile -ExecutionPolicy Bypass -Command "& { & ''' + $PSCommandPath + ''' *>&1 | Out-File -FilePath ''' + $logPath + ''' -Encoding utf8 }"'
...
if ($projectRoot.EndsWith("fmcg-shop-prod")) {
    $prodRoot = $projectRoot
    $uatRoot = Join-Path $parentDir "fmcg-shop"
} else {
    $uatRoot = $projectRoot
    $prodRoot = Join-Path $parentDir "fmcg-shop-prod"
}

$uatEnvPath = Join-Path $uatRoot ".env"
...
$ocrScriptPath = Join-Path $projectRoot "ocr-service\main.py"
...
if (-not $ocrSvc) {
    ...
    & $nssmPath set fmcg-ocr AppDirectory `"$projectRoot\ocr-service`"
} else {
...
$waSecretUat = $uatEnv["WA_INTERNAL_SECRET"]
$backupEncUat = $uatEnv["BACKUP_ENCRYPTION_PASSWORD"]
if ($dbPassUat -and $jwtSecretUat) {
    $envList = @("DB_PASSWORD=$dbPassUat", "JWT_SECRET=$jwtSecretUat")
    if ($waSecretUat) { $envList += "WA_INTERNAL_SECRET=$waSecretUat" }
    if ($geminiApiKey) { $envList += "GEMINI_API_KEY=$geminiApiKey" }
    if ($backupEncUat) { $envList += "BACKUP_ENCRYPTION_PASSWORD=$backupEncUat" }
    & $nssmPath set fmcg-backend-uat AppEnvironmentExtra $envList
...
$waSecretProd = $prodEnv["WA_INTERNAL_SECRET"]
$backupEncProd = $prodEnv["BACKUP_ENCRYPTION_PASSWORD"]
if ($dbPassProd -and $jwtSecretProd) {
    $envList = @("DB_PASSWORD=$dbPassProd", "JWT_SECRET=$jwtSecretProd")
    if ($waSecretProd) { $envList += "WA_INTERNAL_SECRET=$waSecretProd" }
    if ($geminiApiKey) { $envList += "GEMINI_API_KEY=$geminiApiKey" }
    if ($backupEncProd) { $envList += "BACKUP_ENCRYPTION_PASSWORD=$backupEncProd" }
    & $nssmPath set fmcg-backend-prod AppEnvironmentExtra $envList
